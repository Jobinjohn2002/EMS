...
const RequirementTable: React.FC<{ projectId: string }> = ({ }) => {
  ...
  const [editingSubRequirement, setEditingSubRequirement] = useState<SubRequirement | null>(null);
    ...
    setRequirements((prev) =>
      prev.map((req) =>
        req.id === requirementId
          ? { ...req, sub_requirements: [...req.sub_requirements, { id: newSubRequirement.id, sub_requirement: newSubRequirement.subrequirement, status:true }] }
          : req
      )
    );
  ...
  const handleDeleteSubRequirement = async (subRequirementId: number) => {
    try {
      // Call API to soft delete
      await requirementService.updateSubRequirement(subRequirementId, { status: false });
  
      // Update state
      setRequirements(prevRequirements =>
        prevRequirements.map(req => ({
          ...req,
          sub_requirements: req.sub_requirements.map(sub =>
            sub.id === subRequirementId ? { ...sub, status: false } : sub
          )
        }))
      );
    } catch (error) {
      console.error("Failed to soft delete sub-requirement:", error);
    }
  };
  ...
  const handleEditRequirementSave = async (id: number, updatedText: string) => {
    try {
      // Assuming the service has an update method that takes in the updated data
      await requirementService.update(id, { requirement: updatedText }); // Update the requirement with the new text

      // Update the state to reflect the change in the UI
      setRequirements(prevRequirements =>
        prevRequirements.map((req) =>
          req.id === id ? { ...req, requirement: updatedText } : req
        )
      );
    } catch (err) {
  ...
  const handleEditSubRequirementSave = async (id: number, updatedText: string) => {
    try {
      await requirementService.updateSubRequirement(id, { subrequirement: updatedText });
  
      setRequirements(prevRequirements =>
        prevRequirements.map((req) => {
          const updatedSubRequirements = req.sub_requirements.map(sub =>
            sub.id === id ? { ...sub, sub_requirement: updatedText } : sub
          );
          return { ...req, sub_requirements: updatedSubRequirements };
        })
      );
    } catch (err) {
      console.error("Failed to update sub-requirement:", err);
    } finally {
      setShowEditModal(false);
      setEditingSubRequirement(null);
      setEditingRequirement(null);
    }
  };  
    ...
    <div className="p-4">
      ...
      {editingRequirement && (
        <EditMilestoneModal
          ...
          onSave={(text) => handleEditRequirementSave(editingRequirement.id, text)}
        />
      )}

      {editingSubRequirement && (
        <EditMilestoneModal
          isOpen={showEditModal}
          title="Edit Requirement"
          placeholder="Enter requirement"
          defaultValue={editingSubRequirement.sub_requirement}
          onClose={() => setShowEditModal(false)}
          onSave={(text) => handleEditSubRequirementSave(editingSubRequirement.id, text)}
        />
      )}
            ...
            {requirements
              ...
              .map((req) => (
                <React.Fragment key={req.id}>
                  ...
                  <tr className="hover:bg-gray-100 bg-[aliceblue]">
                    ...
                    <td className="px-4 py-3 flex space-x-2">
                      ...
                      <button
                        ...
                        className="text-blue-500 focus:outline-none text-lg bg-transparent"
                        title="Edit Requirement"
                      >
                      ...
                      <button
                        ...
                        className="text-red-500 hover:text-red-700 focus:outline-none text-lg bg-transparent"
                      ...
                      {/* Expand/Collapse Icon - Third */}
                      <button
                        onClick={() => setExpandedId(expandedId === req.id ? null : req.id)}
                        className="text-gray-500 hover:text-gray-700 focus:outline-none text-lg bg-transparent"
                    ...
                    <>
                      ...
                      .map((sub) => (
                        <tr key={sub.id} className="bg-gray-50 hover:bg-gray-100">
                          <td className="px-4 py-3 flex items-center space-x-2 w-12 pl-8">
                            {/* EDIT ICON */}
                            <button
                              onClick={() => {
                                setEditingRequirement(req);      // parent Requirement
                                setEditingSubRequirement(sub);   // the SubRequirement you want to edit
                                setShowEditModal(true);
                              }}
                            ...
                            {/* DELETE ICON */}
                            <button
                              onClick={() => handleDeleteSubRequirement(sub.id)}
                      ...
                      <tr className="bg-gray-50 hover:bg-gray-100">
                        <td className="px-4 py-3 w-12 pl-8"></td>
                        <td className="px-4 py-3 flex items-center space-x-2">
                          ...
                          <button
                            ...
                            disabled={!newSubRequirementText[req.id]?.trim()}
                          >
